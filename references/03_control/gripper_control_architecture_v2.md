...
健康检查不得默认从行程学习结束后的端点回退位置直接起跑。控制器应在已学软件限位内部选择能完成闭合/打开往返的诊断窗口，优先避开已知严重摩擦异常及其安全裕量；如果没有足够干净的内部窗口，应明确失败并输出候选窗口、软件限位、避让阈值和异常数量，而不是用端点附近样本判定整机健康。

健康检查的速度误差、电流纹波和力矩纹波阈值可以由配置默认值、已学习摩擦基线和反馈噪声底共同给出有效下限，用于适配真实硬件反馈的自然波动；但实时硬电流、堵转、接触、方向矛盾和软件限位保护不得因此放宽。失败日志必须输出每段样本的起止位置、方向、速度、实际位移、采样数、最大速度误差、电流/力矩纹波、温度以及本次使用的有效阈值，便于区分阈值过严、诊断窗口选错和真实机构卡滞。
